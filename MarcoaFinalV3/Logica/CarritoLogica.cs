...
namespace MarcoaFinalV3.Logica
{
    public class CarritoLogica
    {
        ...
        public int Registrar(Carrito oCarrito)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("IdProducto", oCarrito.oProductoWeb.IdProducto);
        ...
        public List<CompraWeb> ObtenerCompra(int IdUsuario)
        {
            List<CompraWeb> rptDetalleCompra = new List<CompraWeb>();
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                        ...
                        while (dr.Read())
                        {
                            ...
                            if (doc.Element("DATA") != null)
                            {
                                rptDetalleCompra = (from c in doc.Element("DATA").Elements("COMPRA")
                                                    select new CompraWeb()
                                                    {
                                                        Total = Convert.ToDecimal(c.Element("Total").Value, new CultureInfo("es-PE")),
                                                        FechaTexto = c.Element("Fecha").Value,
                                                        oDetalleCompraWeb = (from d in c.Element("DETALLE_PRODUCTO").Elements("PRODUCTO")
                                                                          select new DetalleCompraWeb()
                                                                          {
                                                                              oProductoWeb = new ProductoWeb()
                                                                              {
                            ...
                            {
                                rptDetalleCompra = new List<CompraWeb>();
                            }
                        }
