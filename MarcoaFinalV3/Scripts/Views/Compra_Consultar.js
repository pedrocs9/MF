...
$(document).ready(function () {
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            $("#cboRestaurant").LoadingOverlay("hide");
            $("#cboRestaurant").html("");

            $("<option>").attr({ "value": 0 }).text("-- Seleccionar todas--").appendTo("#cboRestaurant");
            if (data.data != null)
                $.each(data.data, function (i, item) {

                    if (item.Activo == true) {
                        $("<option>").attr({ "value": item.IdRestaurant }).text(item.Nombre).appendTo("#cboRestaurant");
                    }
                })
        },
        error: function (error) {
        ...
        beforeSend: function () {
            $("#cboRestaurant").LoadingOverlay("show");
        },
    });
    ...
    tabladata = $('#tbCompras').DataTable({
        "ajax": {
            "url": $.MisUrls.url._ObtenerCompras + "?fechainicio=" + ObtenerFecha() + "&fechafin=" + ObtenerFecha() + "&idproveedor=0&idrestaurant=0",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            ...
            {
                "data": "oRestaurant", render: function (data) {
                    return data.Nombre
                }
            },
            { "data": "FechaCompra" },
...
function buscar() {
    ...
    tabladata.ajax.url($.MisUrls.url._ObtenerCompras + "?" +
        ...
        "&idrestaurant=" + $("#cboRestaurant").val()).load();
}
